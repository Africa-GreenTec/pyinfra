...
OPKG_UPDATE_FILENAME = "/tmp/opkg-lists/openwrt_base"


def opkg(command, force=False):
    args = ["opkg"]
...
#             yield "{0} -f".format(opkg("install", force=force))
...
def update(cache_time=None):
    ...
    **Example:**
    ...
    .. code:: python
        ...
        opkg.update(
            name="Update opkg repositories",
            cache_time=3600,
        )
    """

    # If cache_time check when opkg was last updated, prevent updates if within time
    if cache_time:
        # Ubuntu provides this handy file
        cache_info = host.get_fact(File, path=OPKG_UPDATE_FILENAME)
        ...
        if cache_info and cache_info["mtime"] and cache_info["mtime"] > host_cache_time:
            host.noop("opkg list is already up to date")
            return
    ...
    # cache_time to work.
    # if cache_time:
    #     yield "touch {0}".format(OPKG_UPDATE_FILENAME)
...
def packages(
    ...
    **Examples:**
    ...
    yield from ensure_packages(
        ...
        present,
        install_command=opkg(install_command, force=force),
        uninstall_command=opkg(uninstall_command, force=force),
        upgrade_command=opkg(upgrade_command, force=force),
        version_join="=",
        latest=latest,
    )
